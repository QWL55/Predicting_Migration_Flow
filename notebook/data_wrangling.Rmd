...
flow_data <- migration_data %>% 
  ...
  filter(flow > 100, 
         !str_detect(fips_orig, "^072")) # remove Puerto Rico	
...
census_processing <- function(df){
  ...
  df_processed <- df %>% 
    ...
    mutate(
      ...
      EP_age_65 = rowSums(across(B01001_020E:B01001_049E),
...
county_info <- county_demo %>% 
  select(GEOID, tot_pop:pov, EP_age_65, diveristy_idx) %>% 
...
write_csv(county_info,
          "../data/output_data/county_node_info.csv")x
```
...
int_population <- population%>%
  clean_names()%>%
  select(c(country_name, country_code, x2010))%>%
  rename(population = x2010)
```
